<template>
  <div class="search-input-warp" :style="{padding:warpPdpx}">
    ...
    <div class="input-box">
      ...
      >
      <ul class="ul-box" v-if="classData">
        <li class="text-center" v-for="(item,index) in classData" @click.stop="clickSchoolNameFn(item)">
          <span class="text-success textprimary" :class="{gray:index==now}">{{item.className}}</span>
        </li>
      </ul>
    </div>
  </div>
  ...
  export default {
    ...
    methods: {
      searchChildFn () {
        ...
        let params = {
          className:that.keyword
        }
        API.requestClassListApi(params).then((json) => {
          if(json.data.code === "000"){
            that.classData = json.data.data
          }else{
      ...
      clickSchoolNameFn (obj) {
        const that = this
        that.childObj = obj
        that.keywordVal = obj.className
        that.keyword = obj.className
        if(that.newType!==3){
          that.keyword = ''
        }
        that.classData=[]
      ...
      selectDown:function () {
        const that =this
        that.now++;
        if(that.now==that.classData.length){
          that.now=-1;
        }
        if(that.now!=-1){
          that.keyword=this.classData[this.now].className;
          that.keywordVal=this.classData[this.now].className;
          that.childObj = this.classData[this.now]
        }else {
          if(that.newType!==3){
            that.keyword = ''
          }
      ...
      selectUp:function () {
        ...
        that.now--;
        if(that.now==-2){
          that.now=this.classData.length-1;
        }
        if(that.now!=-1){
          that.keyword=this.classData[this.now].className;
          that.keywordVal=this.classData[this.now].className;
